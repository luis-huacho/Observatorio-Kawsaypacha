...
- `09-errores.md` — **el ciclo de errores**: severidades, la regla de «nace con una prueba que falla», qué se hace al cerrar. Los errores abiertos ya no están aquí sino en el **tracker** (Gitea local, ADR-A15): `docker compose -f compose.tracking.yaml up -d` y se gestiona con las herramientas MCP `list_issues` / `issue_read` / `issue_write` de `.mcp.json`. Consultarlo antes de tocar un área y abrir un issue con todo hallazgo que no se arregle en el momento; al corregir uno, se cierra el issue **y** se escribe la entrada de bitácora en `_specs/README.md`
