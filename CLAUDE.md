...
- **Las dos unidades de la distribución no son intercambiables**: "centros poblados por su nivel máximo" (3,238) y "clasificaciones" (10,978) difieren en 3.4×, porque un CCPP aporta una fila por peligro evaluado. Cualquier cifra que se muestre tiene que declarar cuál es. **Conviven en `/peligros`**: la tabla y el panel de distribución cuentan en la primera; el **número de los grupos del visor** en la segunda (ADR-A16, suma de `clasificaciones` del geojson), y el color de los símbolos vuelve a la primera. La cabecera de la tabla muestra las dos para que se puedan cuadrar. `point_count` de MapLibre no sirve ahí: la fuente conserva los que no pasan el filtro para pintarlos en gris, así que contaría lo mismo con filtro y sin él.
