...
| **Optimizar las imágenes ya subidas** | `docker compose exec backend python manage.py optimizar_imagenes --simular` primero, y sin `--simular` cuando el ahorro convenza. Reescala **sin cambiar el formato**, así que las URL publicadas no se mueven. Lo nuevo se optimiza solo al guardarlo (ADR-A25) |
| **Comprobar que las metas de compartir llegan** | `curl -s https://$SITE_DOMAIN/noticias/<slug> \| grep og:title` — **sin navegador a propósito**: es lo que ve WhatsApp. También lo cubre `comprobar-sitio.sh` |
