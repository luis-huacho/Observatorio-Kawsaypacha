...
def a_borrador(apps, schema_editor):
    ...
    apps.get_model("contenidos", "Evento").objects.filter(estado="revision").update(estado="borrador")
    apps.get_model("contenidos", "Noticia").objects.filter(estado="revision").update(estado="borrador")
    apps.get_model("contenidos", "Video").objects.filter(estado="revision").update(estado="borrador")
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('contenidos', '0003_noticia_ia_estado_noticia_log_ia_and_more'),
    ]
