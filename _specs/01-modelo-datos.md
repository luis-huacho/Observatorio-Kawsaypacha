...
### normativa
`Norma (Workflow)`: **`slug` unique**, `titulo`, **FK `tipo`** (null, PROTECT), `ambito` (nacional|regional|local, index), **FK `entidad_emisora`** (null, PROTECT), `fecha` date (index), `resumen`, **`contenido` rich** (el análisis desarrollado de la ficha; `analisis_predes` sigue siendo la nota breve del listado), `url_oficial` null, `analisis_predes` Text null, `imagen_portada` ImageField null, `imagen_titulo` char null, `palabras_clave` ArrayField(char); [+] FK `documento` (biblioteca, el PDF alojado por PREDES), `numero` (p.ej. "DS 048-2011-PCM"), `estado_vigencia` (vigente|derogada|modificada). Hereda además `core.RedaccionIAMixin` — `url_origen`, `ia_estado`, `log_ia` y el candado `redactada_por_ia`— con el que la ficha puede nacer de una URL (ADR-D8); son los mismos cuatro campos que `Noticia`, y por eso viven en un mixin. Desde ADR-D10 ese mixin es **`RedaccionIAMixin(EstadoIAMixin)`**: los tres campos de estado bajan a `EstadoIAMixin` y arriba solo queda `url_origen`, porque `Medida` comparte el estado pero su origen es una ficha, no una URL. La partición **no emitió ninguna migración** — los campos que llegan a `Noticia` y `Norma` son los mismos y el autodetector no reacciona a un cambio de orden.

`TipoNorma`: **`nombre` unique**, `abreviatura` (blank, "DS"), **`slug` unique**, `sinonimos` ArrayField(char), `orden`. Era un `TextChoices` de cinco opciones y ampliarlo exigía migrar y desplegar; pasa a catálogo por lo mismo que `EntidadEmisora` (ADR-D11). Tres cosas que no son obvias: **`sinonimos` es lo que consulta el importador** —sin él, un tipo dado de alta desde el admin entra en el formulario y en la IA pero el importador sigue omitiendo sus filas—; **el slug va en minúscula** (`ds`) aunque el valor crudo fuera `DS`, y el filtro se declara `lookup_expr="iexact"` para que los enlaces `?tipo=DS` ya compartidos sigan filtrando; y **la FK admite nulo** —una clave foránea no guarda la cadena vacía que la IA usa para «no lo sé»—, así que la obligatoriedad se devuelve con `CAMPOS_PARA_PUBLICAR = ("tipo",)`, que además cierra un agujero anterior: una norma redactada por IA con el tipo en blanco sí se podía publicar.

`EntidadEmisora`: **`nombre` unique**, `sigla` (blank, "PCM"), **`slug` unique**, `orden`. Catálogo simple, sin workflow, con la forma de `biblioteca.CategoriaDocumento`. Es taxonomía y no un `CharField` por el motivo de siempre: escrito a mano, «PCM», «Presidencia del Consejo de Ministros» y «P.C.M.» son tres valores y el filtro no sirve. **La FK es PROTECT aunque el campo sea opcional**, apartándose del `SET_NULL` de `Video.tema`: el catálogo se edita desde su propia pantalla de mantenimiento, y con `SET_NULL` borrar una entidad vaciaría en silencio la atribución de todas sus normas. Se siembra desde `normativa/semillas/entidades.yaml` **y** desde la migración de datos `normativa.0005` — el seed no corre en el despliegue (ADR-D11).
...
erDiagram
    ...
    EntidadEmisora ||--o{ Norma : emite
    TipoNorma ||--o{ Norma : clasifica
