...
| `/normativa`, `/normativa/:slug` | `/api/normativa/` + export Excel. **Tres filtros**: ámbito con opciones escritas en el componente (es `TextChoices`), y **tipo y entidad emisora desde `/api/normativa/tipos/` y `/entidades/`**, que es catálogo y no se puede escribir a mano; el desplegable no se pinta si el catálogo llega vacío, porque un select con una sola opción no filtra nada. La tarjeta muestra la **sigla** (el nombre completo no cabe entre los chips) y la ficha el nombre entero, **replegando a «Publicada por el Gobierno …» cuando no consta la entidad** — es el estado de la mayoría de fichas antiguas y quedarse sin la línea sería peor que la aproximación. **La ficha es nueva** (`NormaDetalle.tsx`, ya construida en el prototipo). El acceso a la publicación oficial está en **los dos** sitios vía `EnlaceNorma.tsx`, que resuelve las tres variantes: PDF (descarga), página del portal (enlace externo) y sin enlace registrado. Por eso la tarjeta del listado **no** es un `<Link>` envolvente —tiene dos destinos y anidar anclas es HTML inválido—: enlaza el título y deja el de la norma como hermano. **Los `url_oficial` del prototipo son de ejemplo**; al portar, vienen del dato que carga el editor |
| `/recursos` | `/api/biblioteca/` (deja de ser estático) |
| `/noticias`, `/noticias/:slug` | **Ya construidas en el prototipo** (`Noticias.tsx`, `NoticiaDetalle.tsx`): listado con filtro por tipo y detalle. Portar cambiando el JSON por `/api/noticias/`. **No va en el menú principal** por decisión del dueño del proyecto: se llega desde el bloque de actualidad de la portada y desde la columna "Más" del pie. La ficha cierra con dos bloques de anexos entre el cuerpo y las palabras clave: **«Enlaces relacionados»** (`ListaEnlaces`, extraído de `MedidaDetalle` y compartido por las dos fichas) y **«Documentos»** (`ListaArchivos`, tarjeta con «PDF · 2,3 MB»). El adjunto es un `<a href download>` y **no `BotonDescarga`**: aquel existe por los cuatro segundos que tarda el servidor en *generar* un PDF y por el 429 del límite de 30/hora, y un archivo estático servido por nginx no tiene ni lo uno ni lo otro. **Aquí el estado vacío NO se declara**: sin anexos no se pinta ni el encabezado. La regla de declarar el vacío es para lo que el visitante pidió —una norma sin `url_oficial` dice «Sin enlace oficial registrado» porque la ficha *promete* un acceso al documento—; nadie prometió que una noticia tuviera anexos, y anunciar la ausencia insinuaría un olvido del editor donde no lo hay |
