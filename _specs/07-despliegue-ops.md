...
Separar el sitio público del backend deja el admin y el API fuera del dominio que se difunde, y permite mover cualquiera de los dos sin tocar el otro. El coste es que **el frontend deja de ser mismo-origen**: se activa `django-cors-headers` con allowlist de los dos dominios, y nginx añade cabeceras CORS en `/media/` y `/tiles/` (las necesitan PMTiles por HTTP Range y la exportación del mapa a PNG desde el canvas). Y `CORS_EXPOSE_HEADERS = ["Content-Disposition"]`, porque la interfaz pide las descargas con `fetch` —para poder enseñar que está generando y explicar un 429— y **un `fetch` cross-origin solo lee las cabeceras que el servidor autoriza**: sin esa línea, los PDF y los Excel se guardarían con el identificador del blob en vez de con su nombre. No rompe nada al faltar, que es lo que la hace fácil de perder.
