...
| A15 | Seguimiento de errores: **Gitea local** (`compose.tracking.yaml`), gestionado por el MCP oficial | tabla Markdown en `09-errores.md` · GitHub Issues en `origin` | La tabla duplicaba el estado a mano en tres documentos y ya había dejado una referencia rota desde el código. GitHub Issues costaría cero infraestructura, pero saca la lista de pendientes del control del desarrollador y la publica en un servicio de terceros. Gitea es una herramienta **de desarrollo**: escucha solo en `127.0.0.1`, no se despliega en el servidor y no forma parte del entregable, así que no añade nada que PREDES tenga que operar. Coste asumido: hay que levantar un contenedor para consultar los pendientes. Detalle en 09 |
