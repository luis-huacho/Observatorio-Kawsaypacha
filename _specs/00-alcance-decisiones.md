...
| A15 | Seguimiento de errores: **Gitea** (`compose.tracking.yaml`) en el servidor de desarrollo, publicado solo en `127.0.0.1` y alcanzado por **túnel SSH**; gestionado por el MCP oficial | tabla Markdown en `09-errores.md` · GitHub Issues en `origin` · Gitea en la máquina local | La tabla duplicaba el estado a mano en tres documentos y ya había dejado una referencia rota desde el código. GitHub Issues costaría cero infraestructura, pero saca la lista de pendientes del control del desarrollador y la publica en un servicio de terceros. Vive en el servidor —y no en el portátil— para que haya **un solo tracker**: dos listas de pendientes divergen, que es el problema del que se venía. Sigue sin exponerse: el túnel evita certificado, vhost y superficie pública, y **el servidor es `somosiadigital.com`, el de desarrollo, no la producción de PREDES**, así que no añade nada que ellos tengan que operar. Coste asumido: sin túnel abierto no se consultan los pendientes, y el volumen sqlite queda fuera del servicio de backups, que solo vuelca PostgreSQL. Detalle en 09 |
